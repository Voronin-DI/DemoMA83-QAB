...
@Описание: Экспортный сценарий, который перезаполняет все константы в базе по умолчанию
@ПримерИспользования: И перезаполнение констант значениями по умолчанию
